...
class TopKGate(torch.nn.Module):
    ...
    def apply_on_expert_fn(self, input, ctx, gate_input=None, part_latents=None):
        ...
        latent_dim = 0
        if part_latents is not None:
            gather_ind = critical_data[1][0][:, None, None].repeat(1, 1, part_latents.shape[-1])
            part_latents = torch.gather(part_latents, dim=1, index=gather_ind.long()).squeeze(1)
            input = torch.cat([input, part_latents], -1)
            latent_dim = part_latents.shape[-1]
            

        # if not hasattr(self, '_fdr'):
        self._fdr = fast_dispatcher(num_global_experts=g_experts, capacity=capacity, model_dim=M, latent_dim=latent_dim, dispatch_dtype=input.dtype)
        ...
        if ctx.sharded_count > 1:
            ...
            dispatched_input = dispatched_input.reshape(world_size, -1, capacity, M + latent_dim)

        ## a2a_ffn_overlap_degree = 1, use_2d_a2a=False
        if self.a2a_ffn_overlap_degree == -1:
            ...
            # print(dispatched_input.shape, gates.shape)        ## torch.Size([1, 4, 30720, 256]) torch.Size([122880, 4])
            expert_output = ctx.expert_fn(dispatched_input)
            expert_output = expert_output.to(input.dtype)
    ...
    def apply_on_expert_fn_nobatch(self, input, ctx, gate_input=None, part_latents=None):
        # input = input.to(next(iter(ctx.experts.parameters())).dtype)
        if self.input_dropout:
            input = self.input_dropout(input)
        ...
        self._fdr_nobatch = fast_dispatcher_nobatch(num_global_experts=g_experts, capacity=capacity, model_dim=M, latent_dim=latent_dim, dispatch_dtype=input.dtype)
        ...
        if ctx.sharded_count > 1:
            ...
            dispatched_input = dispatched_input.reshape(-1, M + latent_dim)
...
class MOELayer(torch.nn.Module):
    ...
    def forward(self, input: Tensor, gate_index=0, **kwargs):
        ...
        if ("apply_on_expert_fn_name" in kwargs) and (kwargs["apply_on_expert_fn_name"] is not None):
            ...
            part_latents = kwargs["part_latents"] if "part_latents" in kwargs else None
            if self.moe_no_batch:
                result_output, l_aux, gate_extras = self.gates[gate_index].apply_on_expert_fn_nobatch(reshaped_input, self, gate_input=reshaped_gate_input, part_latents=part_latents)
            else:
                result_output, l_aux, gate_extras = self.gates[gate_index].apply_on_expert_fn(reshaped_input, self, gate_input=reshaped_gate_input, part_latents=part_latents)
...
class ExpertMLP(torch.nn.Module):
    ...
    def __init__(self, model_dim, local_experts, layer_num, skips=None, activation=F.relu, init_factor=1.0, init_trunc_normal=False, latent_dim=0):
        super().__init__()
        self.model_dim = model_dim
        self.local_experts = local_experts
        ...
        self.coded_dim = self.model_dim + latent_dim
        self.coded_layer = [0] + self.skips

        self.weights = nn.ParameterList()
        self.bias = nn.ParameterList()

        for j in range(self.layer_num):
            coded_model_dim = self.coded_dim if j in self.coded_layer else self.model_dim
            fc1_weight = nn.Parameter(torch.zeros(local_experts, coded_model_dim, self.hidden_dim))
            fc1_bias = nn.Parameter(torch.zeros(local_experts, 1, self.hidden_dim))
            for i in range(local_experts):
                fc1 = nn.Linear(coded_model_dim, self.hidden_dim)
                if self.init_trunc_normal:
                    trunc_normal_linear(fc1, std=init_factor)
                    with torch.no_grad():
    ...
    def reset_parameters(self):
        for j in range(self.layer_num):
            ...
            for i in range(self.local_experts):
                fc1 = nn.Linear(self.coded_dim if j in self.coded_layer else self.model_dim, self.hidden_dim)
                if self.init_trunc_normal:
    ...
    def forward(self, x, ctx):
        ...
        if self.latent_dim != 0:
            x, part_latents = x.split([self.model_dim, self.latent_dim], -1)
        h = x
        for layer_id in range(self.layer_num):
            ...
            if self.latent_dim != 0 and self.skips is not None and layer_id in self.coded_layer:
                h = torch.cat([h, part_latents], -1)
...
class SingleExpert(torch.nn.Module):
    # one layer MLP with experts
    def __init__(self, model_dim, layer_num, skips=None, activation=F.relu, init_factor=1.0, norm_layer=nn.LayerNorm, use_norm=False, init_trunc_normal=False, latent_dim=0):
        super().__init__()
        self.model_dim = model_dim
        self.layer_num = layer_num
        self.hidden_dim = model_dim
        ...
        for i in range(self.layer_num):
            coded_model_dim = self.coded_dim if i in self.coded_layer else self.model_dim
            layer = nn.Linear(coded_model_dim, self.model_dim)
            if self.init_trunc_normal:
