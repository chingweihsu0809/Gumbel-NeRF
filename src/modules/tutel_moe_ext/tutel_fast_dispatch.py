...
class GatingEncoder(torch.autograd.Function):
    ...
    def forward(ctx: Any, config: Any, reshaped_input: Tensor, *gates_):
        ...
        dispatched_input = torch.zeros([ctx.config.num_global_experts * ctx.config.capacity, ctx.config.model_dim + ctx.config.latent_dim], dtype=reshaped_input.dtype, device=reshaped_input.device)
        for g, i, l in zip(ctx.gates_h2, ctx.config.indices_, ctx.config.locations_):
          ctx.config.func_fwd(g, i, l, reshaped_input, dispatched_input, extra=[ctx.config.indices_[0].size(0), ctx.config.aligned_dim + ctx.config.latent_dim, ctx.config.capacity])
    ...
    def backward(ctx: Any, dispatched_input: Tensor):
        ...
        for g, i, l in zip(ctx.gates_h2, ctx.config.indices_, ctx.config.locations_):
          ...
          ctx.config.func_bwd_data(g, i, l, grad_data, dispatched_input, extra=[ctx.config.indices_[0].size(0), ctx.config.aligned_dim + ctx.config.latent_dim, ctx.config.capacity])
        ...
        if id(ctx.gates_h2[0]) != id(ctx.config.ones_helper):
          for i, l in zip(ctx.config.indices_, ctx.config.locations_):
            ...
            ctx.config.func_bwd_gate(grad_gates1_s, i, l, ctx.reshaped_input, dispatched_input, extra=[ctx.config.indices_[0].size(0), ctx.config.aligned_dim + ctx.config.latent_dim, ctx.config.capacity])
...
class TutelMoeFastDispatcher:
    ...
    def __init__(self, num_global_experts, capacity, model_dim, latent_dim, dispatch_dtype):
...
def extract_critical_load_importance(gates, gates_wo_noise, logits_w_noise, top_k, gate_noise, capacity_factor=1.0, fp32_gate=False, batch_prioritized_routing=False, compute_balance_loss=False):
    ...
    if capacity_factor > 0:
        ...
        if capacity_factor < 0:
            capacity = min(capacity, top_k * int(-capacity_factor * ((int(gates.size(0)) + num_global_experts - 1) // num_global_experts)))
    return (num_global_experts, indices_s, locations_s, gates_s, capacity), l_loss, l_balance_loss
