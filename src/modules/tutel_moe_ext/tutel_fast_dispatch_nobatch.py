...
class GatingEncoder(torch.autograd.Function):
    ...
    def forward(ctx: Any, config: Any, reshaped_input: Tensor, *gates_):
        ...
        dispatched_input = torch.zeros([dispatched_input_numel, ctx.config.model_dim + ctx.config.latent_dim], dtype=reshaped_input.dtype, device=reshaped_input.device)
        for g, i, l in zip(ctx.gates_h2, ctx.config.indices_, ctx.config.locations_):
          ctx.config.func_fwd(g, i, l, expert_locations_begin, reshaped_input, dispatched_input, extra=[ctx.config.indices_[0].size(0), ctx.config.aligned_dim + ctx.config.latent_dim, ctx.config.capacity])
    ...
    def backward(ctx: Any, dispatched_input: Tensor):
        ...
        for g, i, l in zip(ctx.gates_h2, ctx.config.indices_, ctx.config.locations_):
          ...
          ctx.config.func_bwd_data(g, i, l, expert_locations_begin, grad_data, dispatched_input, extra=[ctx.config.indices_[0].size(0), ctx.config.aligned_dim + ctx.config.latent_dim, ctx.config.capacity])
        ...
        if id(ctx.gates_h2[0]) != id(ctx.config.ones_helper):
          for i, l in zip(ctx.config.indices_, ctx.config.locations_):
            ...
            ctx.config.func_bwd_gate(grad_gates1_s, i, l, expert_locations_begin, ctx.reshaped_input, dispatched_input, extra=[ctx.config.indices_[0].size(0), ctx.config.aligned_dim + ctx.config.latent_dim, ctx.config.capacity])
