...
def get_rgb_index_mask(metadata: ImageMetadata) -> Optional[
    ...
    assert metadata.image_index <= torch.iinfo(torch.long).max
    return rgbs, metadata.image_index * torch.ones(rgbs.shape[0], dtype=torch.long), metadata.instanceid * torch.ones(rgbs.shape[0], dtype=torch.long), keep_mask
