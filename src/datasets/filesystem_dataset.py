...
from src.datasets.dataset_utils import get_rgb_index_mask
from src.image_metadata import ImageMetadata
from src.misc_utils import main_tqdm, main_print, main_log, process_log
from src.ray_utils import get_ray_directions, get_rays, get_rays_batch
...
class FilesystemDataset(Dataset):
    ...
    def __init__(self, metadata_items: List[ImageMetadata], near: float, far: float,
        ...
        if append_arrays is not None:
            ...
            self._id_arrays = append_arrays[3]
        else:
            self._rgb_arrays = []
            self._ray_arrays = []
            self._img_arrays = []
        ...
        self._id_arrays.sort(key=lambda x: x.name)
    ...
    def load_chunk(self) -> None:
        chosen, self._loaded_rgbs, self._loaded_rays, self._loaded_image_indices, self._loaded_id = self._chunk_future.result()
        self._chosen = chosen
        next_chunk_index = next(self._chunk_index)
        self._chunk_future = self._chunk_load_executor.submit(partial(self._load_chunk_inner, next_chunk_index))
    ...
    def __getitem__(self, idx) -> Dict[str, torch.Tensor]:
        return {
            ...
            'id': self._loaded_id[idx]
        }

    def _load_chunk_inner(self, next_index) -> Tuple[
        str, torch.FloatTensor, torch.FloatTensor, torch.LongTensor]:
        if 'RANK' in os.environ:
            torch.cuda.set_device(int(os.environ['LOCAL_RANK']))

        # next_index = next(self._chunk_index)
        chosen = self._rgb_arrays[next_index]
        loaded_img_indices = torch.LongTensor(np.load(self._img_arrays[next_index]))
        loaded_id = torch.LongTensor(np.load(self._id_arrays[next_index]))
        ...
        return str(chosen), torch.FloatTensor(np.load(chosen)) / 255., loaded_rays, loaded_img_indices, loaded_id
    ...
    def _write_chunks(self, metadata_items: List[ImageMetadata], center_pixels: bool, device: torch.device,
        ...
        for chunk_path in chunk_paths:
            ...
            (chunk_path / 'id-chunks').mkdir(parents=True)
        ...
        for chunk_path, path_free in zip(chunk_paths, path_frees):
            ...
            for j in range(allocated):
                ...
                id_array_path = chunk_path / 'id-chunks' / '{}.npy'.format(index)
                self._id_arrays.append(id_array_path)
                id_append_arrays.append(NpyAppendArray(str(id_array_path)))
                index += 1
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            for metadata_item in main_tqdm(metadata_items):
                ...
                image_rgbs, image_indices, image_id, image_keep_mask = image_data
                ...
                if self._directions is not None:
                    ...
                    directions = get_ray_directions(metadata_item.W,
                                                    ...
                                                    center_pixels,
                                                    device)
                    image_rays = get_rays(directions, metadata_item.c2w.to(device), self._near, self._far).view(-1, 8).cpu()
                ...
                if in_memory_count >= disk_flush_size:
                    ...
                    write_futures = self._write_to_disk(executor, torch.cat(rgbs), torch.cat(rays), torch.cat(indices), torch.cat(id),
                                                        rgb_append_arrays, ray_append_arrays, img_append_arrays, id_append_arrays)
        ...
        for chunk_path in chunk_paths:
            if chunk_path.exists():
                ...
                for child in list((chunk_path / 'rgb-chunks').iterdir()):
                    ...
                    id_arrays.append(child.parent.parent / 'id-chunks' / child.name)
                num_exist += 1

        if num_exist > 0:
            assert num_exist == len(chunk_paths)
            return rgb_arrays, ray_arrays, img_arrays, id_arrays
        else:
            return None

    @staticmethod
    def _write_to_disk(executor: ThreadPoolExecutor, rgbs: torch.Tensor, rays: torch.FloatTensor,
                       image_indices: torch.Tensor, ids: torch.Tensor, 
                       rgb_append_arrays, ray_append_arrays, img_append_arrays, id_append_arrays):
        ...
        def append(index: int) -> None:
            ...
            id_append_arrays[index].append(ids[indices[index * chunk_size:(index + 1) * chunk_size]].numpy())
