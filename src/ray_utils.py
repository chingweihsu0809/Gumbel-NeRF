...
def get_rays(directions: torch.Tensor, c2w: torch.Tensor, near: float, far: float) -> torch.Tensor:
    ...
    return _get_rays_inner(rays_o, rays_d, near, far)


def get_rays_batch(directions: torch.Tensor, c2w: torch.Tensor, near: float, far: float) -> torch.Tensor:
    # Rotate ray directions from camera coordinate to the world coordinate
...
def _get_rays_inner(rays_o: torch.Tensor, rays_d: torch.Tensor, near: float, far: float) -> torch.Tensor:
    # c2w is drb
